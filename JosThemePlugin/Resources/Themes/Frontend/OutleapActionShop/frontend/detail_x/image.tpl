...
{block name="frontend_detail_image"}
	{if $theme.artikel_images_position}
										...
										<span class="image--media">
											{if isset($sArticle.image.thumbnails)}
												{block name='frontend_detail_image_default_picture_element'}
													<img srcset="{$sArticle.image.thumbnails[1].sourceSet}" src="{$sArticle.image.thumbnails[1].source}" alt="{$alt}" itemprop="image" />
												{/block}
												{if $theme.detail_image_hinweistext_show}<div id="tci-hinweistext">{s name='DetailImageHinweistext'}Abbildung ähnlich{/s}</div>{/if}
											{else}

...
{block name='frontend_detail_configurator_variant'}
	{if !$activeConfiguratorSelection && $theme.artikel_first_select_variant_info_show}
		<div class="firstselectvariantinfo">{s name="firstSelectVariantInfo" namespace="frontend/detail/data"}Bitte wähle zuerst eine Variante{/s}</div>
	{/if}
                            ...
                            {block name='frontend_detail_configurator_variant_group_options'}
                                {foreach $configuratorGroup.values as $option}
                                    ...
                                    {block name='frontend_detail_configurator_variant_group_option'}
                                        <div class="variant--option{if $option.media} is--image{/if}{if !$sArticle.notification && !$option.selectable && $theme.artikel_hide_inaktive_varianten} is--hidden{/if}">

                                            {block name='frontend_detail_configurator_variant_group_option_input'}
                                                <input type="radio"
                                                       class="option--input{if !$option.selectable && $theme.artikel_hide_inaktive_varianten} is--hidden{/if}"
                                            ...
                                            {block name='frontend_detail_configurator_variant_group_option_label'}
                                                <label for="group[{$option.groupID}][{$option.optionID}]" class="option--label{if !$sArticle.notification && !$option.selectable} is--disabled{/if}{if !$option.selectable && $theme.artikel_hide_inaktive_varianten} is--hidden{/if}">
                                                    {if $option.media}
                                                        {$media = $option.media}
